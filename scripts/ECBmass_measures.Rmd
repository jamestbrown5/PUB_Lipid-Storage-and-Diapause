...
This is an [R Markdown](https://urldefense.proofpoint.com/v2/url?u=http-3A__rmarkdown.rstudio.com&d=DwIGAg&c=sJ6xIWYx-zLMB3EPkvcnVg&r=Kyb6I8wzvFcpJGz2ZzK6rd1ksol3UOMP-IlMyi_b4dU&m=2AMKdOl9sZMSXPN-UppAwjeL7hB10IbCzj8Fuc1PF28&s=HoRyle3LB7p4QTRPoj6Pd26uv-Un77befmmRTDhJ7pM&e= ) Notebook. When you execute code within the notebook, the results appear beneath the code. 
...
## Plots and data analysis of CO2 production. IMPORT "Respir.xlsx" DATASET FIRST!!!
```{r}

Respir <- read_excel("~/GitHub/Respirometry/Data/Respir.xlsx")
Respir<-read_excel("../Data/Respir.xlsx")
data=Respir
############## Data Prep ###################
...
ggplot(dat2,aes(x=Photoperiod,y=mass.mg,fill=strain))+geom_boxplot()+ylab("Wet Mass (mg)")+xlab("Photoperiod (hours)")+scale_fill_manual(labels=c("Long Diapause","Short Diapause"),values=c("lightsalmon1","mediumpurple1"),breaks=c("UZ","BE"))+scale_y_continuous(breaks=seq(0,160,20),labels=seq(0,160,20),limits = c(0,160))#+T



###Stats of Peakwet mass day by strain
dat2%>%
  group_by(Photoperiod,strain,Tray)%>%
  dplyr::summarise(n=length(Tray))
  

comp=aov(mass.mg ~ Photoperiod * strain + Error(Photoperiod/Tray)  ,data=dat)
summary(comp)
...
data<-read_excel("../Data/Extraction Samples JLM.xlsx")
...
###wet mass, lean mass, lipid mass; 
#statistical model: testing effects of strain and photoperiod
...
data<-read_excel("../Data/Extraction Samples JLM.xlsx",sheet="CONTSAMPdf")
...
dataL1$photoperiod<-substr(dataL1$treat,3,4)
dataL1$strain<-substr(dataL1$treat,1,2)
...
mixedL1.2=lmer(wet_mass ~ photoperiod*strain+lean_mass + (1|rep/cohort) ,data=dataL1, REML = TRUE)
lmerTest::step(mixedL1.2)
#summary(mixedL1.2)

mixedL1=lmer(lipid_mass ~ photoperiod*strain+lean_mass + (1|rep/cohort) ,data=dataL1, REML = TRUE,control=lmerControl(check.conv.singular = .makeCC(action = "ignore",  tol = 1e-4)))
summary(mixedL1)
lmerTest::step(mixedL1)
#summary(mixedL1)


mixedL1.3=lmer(lean_mass ~ photoperiod*strain+(1|rep/cohort) ,data=dataL1, REML = TRUE)
...
d1_lm<-ggplot(dataL1,aes(x=photoperiod,y=lipid_mass,fill=strain))+geom_boxplot()+scale_fill_manual(labels=c("Long Diapause","Short Diapause"),values=c("lightsalmon1","mediumpurple1"),breaks=c("UZ","BE"))+T+ylab("Lipid Mass (g)")+xlab("Photoperiod")+facet_wrap(~sample_day)
d1_lm
...
ggplot(d.long3,aes(x=photoperiod,y=mg,fill=strain))+facet_wrap(mt.fac~facet,scales="free_y",ncol=2)+geom_boxplot()+T+ylab("Mass (mg)")+xlab("Photoperiod (hours)")+scale_fill_manual(labels=c("Long Diapause","Short Diapause"),values=c("lightsalmon1","mediumpurple1"),breaks=c("UZ","BE"))+scale_y_continuous(breaks = scales::pretty_breaks(5), limits = c(0, NA))#+scale_y_continuous(breaks=seq(0,200,5),labels=seq(0,200,5))
...
#data%>%
  #group_by(sample_day)%>%
  #dplyr::summarise(n=length(sample_day))

dataLW$d1_lm<-dataL1$lipid_mass
#this model attempts to fit the linear mixed effects model with 3 variables; photoperiod, lipid mass, ##Mixed model with 'rep' as a random factor
...
#Predicting the day wet mass peaks
```{r}
data=read_excel("/Users/jbrown/Documents/GitHub/Respirometry/Data/Respir.xlsx", sheet="Sheet1")
...
step(mixed1,reduce.random=FALSE)
...
## Plots and data analysis for DRY MASS minus LIPID MASS during diapause preparation
```{r}
#################### Data Prep ##########################
##PC
data=read_excel("/Users/jbrown/Documents/GitHub/Lipid_Quantification/Data/Extraction Samples JLM.xlsx",sheet = "CONTSAMPdf")
...
#Reducing df
data1=data[1:626, c("sample_day","rep", "treat","sample_id","5th_date","cohort", "wet_mass","dry_mass","calc_lean_mass","lipid_mass","strain","photoperiod")]
...
#Larva Lean Mass Plot
datacLeU=data1
#datacLeU=subset(datacLeU,treat=="UZ12"|treat=="UZ16")
#datacLeU=subset(datacLeU,calc_lean_mass>=0.0001)
#datacLeU=subset(datacLeU,calc_lean_mass<=0.1000)
count(datacLeU$treat)
datacLeU=subset(datacLeU,sample_day=="1" | sample_day=="W")
#datacLeU=subset(datacLeU,photoperiod=="16")

## Plot comparing lean mass accumulation 
ggplot(data=datacLeU,aes(x=sample_day,y=calc_lean_mass,color=treat,group=treat))+
  stat_summary(fun.y=mean, geom="line")+
  stat_summary(fun.data=mean_se, geom="errorbar", width=0.05) +
  theme(axis.text.x  = element_text(vjust=0.5, size=16),axis.text.y  = element_text(vjust=0.5, size=16))+
  #expand_limits(y=c(0.01,0.035))+
  scale_color_manual(values=c(UZ12="red",UZ16="red",BE12="blue",BE16="blue"))+
  ylab("Calculated Lean Mass")+xlab("Sample Day")
...
### Is there a significant difference in lean mass accumulation between treatments in preparation for diapause?
datacL1=subset(data,sample_day=="1")
#datacL1=subset(datacL1,calc_lean_mass>="0")
#datacL1=subset(datacL1,calc_lean_mass<="0.07")

mod=lm(calc_lean_mass ~ photoperiod*strain, data=datacL1, REML = TRUE)
summary(mod)

mixed=lmer(calc_lean_mass ~ photoperiod*strain + (1|rep/cohort) ,data=datacL1, REML = TRUE)
mixedcL2=lmer(calc_lean_mass ~ photoperiod + (1|rep/cohort) ,data=datacL1, REML = TRUE)
mixedcL3=lmer(calc_lean_mass ~ strain + (1|rep/cohort) ,data=datacL1, REML = TRUE)
mixedcL4=lmer(calc_lean_mass ~ (1|rep/cohort) ,data=datacL1, REML = TRUE)
anova(mixedcL2,mixedcL3,mixedcL4,mixed)

#this model attempts to fit the linear mixed effects model with 3 variables; photoperiod, lean mass, and the random effect of rep
## Photoperiod along Does NOT SIGNIFICANTLY effect lean mass accumulation between larvae reared in long day vs short day conditions
mixedcL1=lmer(calc_lean_mass ~ strain*photoperiod + (1|rep/cohort) ,data=datacL1, REML = TRUE)
step(mixedcL1, reduce.random=FALSE)
summary(mixedcL1)
plot(mixedcL1)
##The following model removes photoperiod as factors to determine the effect of lean mass and Rep on lean mass
submodcL1=lmer(calc_lean_mass ~ (1|rep/cohort),data=datacL1, REML = TRUE)
summary(submodcL1)

## Standard Residuals -  Adding stdres to df and checking for regularity of distribution within +/-2 standard deviations  
datacL1$stdrescL1<-as.vector(scale(resid(submodcL1)))
...
datacLW=subset(data,sample_day=="W")
datacLW=subset(datacLW,calc_lean_mass>="0")
datacLW=subset(datacLW,calc_lean_mass<="0.07")

#this model attempts to fit the linear mixed effects model with 3 variables; photoperiod, lean mass, with rep as a random factor
## Photoperiod does SIGNIFICANTLY effect lean mass accumulation between larvae reared in long day vs short day conditions
mixedcLW=lmer(calc_lean_mass ~ strain+photoperiod + (1|rep/cohort) ,data=datacLW, REML = TRUE)
summary(mixedcLW)
##The following model removes photoperiod as factors to determine the effect of lean mass and Rep on lean mass
submodcLW=lmer(calc_lean_mass ~ (1|rep/cohort),data=datacLW, REML = TRUE)
summary(submodcLW)

## Standard Residuals -  Adding stdres to df and checking for regularity of distribution within +/-2 standard deviations  
datacLW$stdrescLW<-as.vector(scale(resid(submodcLW)))


mixed8=lmer(calc_lean_mass ~ photoperiod*strain + (1|rep/cohort) ,data=datacLW, REML = TRUE)
mixed2=lmer(calc_lean_mass ~ photoperiod+strain + (1|rep/cohort) ,data=datacLW, REML = TRUE)
mixedcL6=lmer(calc_lean_mass ~ photoperiod + (1|rep/cohort) ,data=datacLW, REML = TRUE)
mixedcL7=lmer(calc_lean_mass ~ strain + (1|rep/cohort) ,data=datacLW, REML = TRUE)
mixedcL8=lmer(calc_lean_mass ~ (1|rep/cohort) ,data=datacLW, REML = TRUE)
anova(mixedcL6,mixedcL7,mixedcL8,mixed2,mixedcL8)
```

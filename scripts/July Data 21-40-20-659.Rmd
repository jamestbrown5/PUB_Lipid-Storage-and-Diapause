...
## all traits against cohort color coded by cohort with points
ggplot(dat.long,aes(x=Cohort,y=measurement,colour=factor(Cohort)))+geom_point()+facet_grid(traits~.,scales="free")
...
mod1=aov(Lipid.Wt~Season*Cohort+Block+LEAN.Larvae,data=data)
summary(mod1)
`````
...
### Point Plots
ggplot(data,aes(x=Season,y=Lipid.Wt,colour=Cohort))+geom_point()+geom_line(data=mean.dat,aes(x=as.numeric(Season),y=mean,colour=Cohort))
````